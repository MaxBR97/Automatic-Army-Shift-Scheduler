...
function inferLastPatrolIntervalFromHistory(history: any) {
    let intervalSolela:number = 0, intervalShinGimel:number = 0;
...
function adjustCommandingTimesAccordingToRestNeeded(patrolDateTime:Date, indexOfSoldier: number, jsonData) {
    ...
    if(restTimeUntilPlanFrom < minShiftsBreak*patrolIntervals) {
        let commandFrom:Date = new Date(planFrom.getTime() - (60*1000)); //reduce one minute
        let commandUntil:Date = addMinutesToDate(planFrom, (minShiftsBreak*patrolIntervals)-restTimeUntilPlanFrom);
